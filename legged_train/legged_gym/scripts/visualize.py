...
exptid = args.exptid
data_raw = np.load('./data/latent.npy', allow_pickle=True)
features = data_raw[:, :-1]
labels = data_raw[:, -1]
...
np.save('./data/T-SNE.npy', tsne_features)
fig, ax = plt.subplots()
...
for label in unique_labels:
    ...
    ax.scatter(x, y, label=label, alpha=0.2,s=8, marker="o")
    if i == 12:
        break
    i+= 1
ax.legend()
...
# # 使用Plotly创建交互式散点图
# fig = px.scatter(df, x='x', y='y', color='label', title=f'{exptid} t-SNE Visualization')
