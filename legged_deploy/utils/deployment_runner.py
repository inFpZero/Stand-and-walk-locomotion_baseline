...
from legged_deploy.utils.logger import MultiLogger
...
class DeploymentRunner:
    ...
    def calibrate(self, wait=True, low=False):
        ...
        for agent_name in self.agents.keys():
            if hasattr(self.agents[agent_name], "get_obs"):
                ...
                #         break
                input("tab enter to calibrate, the robot will reach initial pos")
                time.sleep(1)
                ...
                for target in reversed(target_sequence):
                    next_target = target
                    # print(next_target)
                ...
                input("tab enter to start controller, the robot will reach initial pos")
                time.sleep(1)
    ...
    def run(self, num_log_steps=1000000000, max_steps=100000000, logging=True):
        ...
        actions_file_path = '/home/robot/actions_v=0.5.json'
        # 使用json模块加载动作列表
        with open(actions_file_path, 'r') as file:
            actions_list = json.load(file)

        # now, run control loop
        try:
            for i in range(max_steps):
                
                # 假设actions.json文件路径已知
                # action = torch.tensor(actions_list[i], dtype=torch.float32)
                action = self.policy(control_obs)

                # if i == 0:
                #     action_pre = action
                # action = action_pre * 0.2 + 0.8 * action
                # action_pre = action
                # plot_action.appensd(action[0][1].item())
                # plot_steps.append(i)
                # if i == 300 :
                #     plt.clf()
                #     plt.xlabel('steps')
                #     plt.ylabel('action1')
                #     plt.plot(plot_steps, plot_action, color='skyblue', label='action')
                #     plt.legend()
                #     plt.savefig('deploy_policy.jpg')
                #     print("image of actions have been saved")
