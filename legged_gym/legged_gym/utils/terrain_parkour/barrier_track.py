...
class BarrierTrack:
    ...
    def get_leap_track(self,
        ...
        if isinstance(self.track_kwargs["leap"]["depth"], (tuple, list)):
            if difficulty is None:
                leap_depth = np.random.uniform(*self.track_kwargs["leap"]["depth"])
            else:
                leap_depth = (1-difficulty) * self.track_kwargs["leap"]["depth"][0] + difficulty * self.track_kwargs["leap"]["depth"][1]
        else:
            leap_depth = self.track_kwargs["leap"]["depth"]
        length_px = int(leap_depth / self.cfg.horizontal_scale)
        ...
        block_info = torch.tensor([
            leap_depth + self.track_kwargs["leap"].get("fake_offset", 0.), # along x(forward)-axis
